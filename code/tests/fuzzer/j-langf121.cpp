    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( WriteData ( fd_Afoo, 35768, 32768) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 35768 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 68536, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				if ( WriteData ( fd_Afoo, 66536, 5000) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 134072) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);

				if ( fallocate( fd_foo , FALLOC_FL_KEEP_SIZE , 166840 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 166840 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE , 199608 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
