    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , 0 , 63536 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 66536 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( WriteData ( fd_Afoo, 104304, 32768) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}


				if ( WriteData ( fd_Afoo, 137072, 32768) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 137072 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 202608 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
