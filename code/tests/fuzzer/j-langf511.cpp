    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_KEEP_SIZE , 33768 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( WriteData ( fd_foo, 36768, 5000) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 38768, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				if ( pwrite ( fd_foo, data_foo, 32768, 140072) < 0){
					cm_->CmClose( fd_foo); 
					return errno;
				}
				cm_->CmClose(fd_foo);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE , 197608 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
