    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 163840 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 196608 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 229376) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);
