    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 131072, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				if ( WriteData ( fd_Afoo, 102304, 5000) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				char *filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 104304, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 137072) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE , 167840 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
