    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( pwrite ( fd_foo, data_foo, 32768, 98304) < 0){
					cm_->CmClose( fd_foo); 
					return errno;
				}
				cm_->CmClose(fd_foo);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 66536 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , 0 , 71536 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				char *filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 71536, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				...
				while (moffset_Afoo < 32768){
					if (to_write_Afoo < 32){
						memcpy(filep_Afoo + 71536 + moffset_Afoo, mtext_Afoo, to_write_Afoo);
						moffset_Afoo += to_write_Afoo;
					}
					else {
						memcpy(filep_Afoo + 71536 + moffset_Afoo,mtext_Afoo, 32);
						moffset_Afoo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_Afoo + 71536, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_Afoo,71536 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_Afoo , 71536 + 32768);
				...
				if ( WriteData ( fd_foo, 194608, 5000) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				if ( fallocate( fd_foo , 0 , 229376 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 104304 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 262144 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( WriteData ( fd_foo, 292912, 5000) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				if ( fallocate( fd_Afoo , 0 , 200608 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
