    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 101304, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				if ( pwrite ( fd_foo, data_foo, 32768, 232376) < 0){
					cm_->CmClose( fd_foo); 
					return errno;
				}
				cm_->CmClose(fd_foo);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 98304 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 129072 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( WriteData ( fd_Afoo, 132072, 5000) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
