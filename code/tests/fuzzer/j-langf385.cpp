    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_Afoo , 0 , 35768 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				char *filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 35768, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				...
				while (moffset_Afoo < 32768){
					if (to_write_Afoo < 32){
						memcpy(filep_Afoo + 35768 + moffset_Afoo, mtext_Afoo, to_write_Afoo);
						moffset_Afoo += to_write_Afoo;
					}
					else {
						memcpy(filep_Afoo + 35768 + moffset_Afoo,mtext_Afoo, 32);
						moffset_Afoo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_Afoo + 35768, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_Afoo,35768 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_Afoo , 35768 + 32768);
				...
				if ( WriteData ( fd_Afoo, 68536, 32768) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if ( WriteData ( fd_foo, 163840, 32768) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
