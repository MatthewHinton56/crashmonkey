    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE , 98304 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 68536 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 163840, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE , 194608 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 297912) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);
				...
				if ( fallocate( fd_foo , FALLOC_FL_KEEP_SIZE , 197608 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
