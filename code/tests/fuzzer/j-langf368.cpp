    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 68536 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( WriteData ( fd_foo, 99304, 5000) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 101304 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 134072, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 131072 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( WriteData ( fd_foo, 197608, 5000) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 229376 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
