    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , 0 , 104304 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				 filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 104304, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				if (filep_foo == MAP_FAILED) {
					 return -1;
				}
				...
				while (moffset_foo < 32768){
					if (to_write_foo < 32){
						memcpy(filep_foo + 104304 + moffset_foo, mtext_foo, to_write_foo);
						moffset_foo += to_write_foo;
					}
					else {
						memcpy(filep_foo + 104304 + moffset_foo,mtext_foo, 32);
						moffset_foo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_foo + 104304, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_foo,104304 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_foo , 104304 + 32768);
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE , 202608 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( WriteData ( fd_foo, 235376, 32768) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				if ( fallocate( fd_foo , 0 , 268144 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 294912) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);
				...
				if ( WriteData ( fd_foo, 300912, 32768) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				if ( fallocate( fd_Afoo , 0 , 327680 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
