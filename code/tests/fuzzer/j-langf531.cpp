    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 99304 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , 0 , 102304 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , 0 , 107304 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				char *filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 107304, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				...
				while (moffset_Afoo < 32768){
					if (to_write_Afoo < 32){
						memcpy(filep_Afoo + 107304 + moffset_Afoo, mtext_Afoo, to_write_Afoo);
						moffset_Afoo += to_write_Afoo;
					}
					else {
						memcpy(filep_Afoo + 107304 + moffset_Afoo,mtext_Afoo, 32);
						moffset_Afoo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_Afoo + 107304, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_Afoo,107304 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_Afoo , 107304 + 32768);
				...
				if ( WriteData ( fd_Afoo, 140072, 32768) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_KEEP_SIZE , 163840 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , 0 , 172840 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				 filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 172840, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				if (filep_Afoo == MAP_FAILED) {
					 return -1;
				}
				...
				while (moffset_Afoo < 32768){
					if (to_write_Afoo < 32){
						memcpy(filep_Afoo + 172840 + moffset_Afoo, mtext_Afoo, to_write_Afoo);
						moffset_Afoo += to_write_Afoo;
					}
					else {
						memcpy(filep_Afoo + 172840 + moffset_Afoo,mtext_Afoo, 32);
						moffset_Afoo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_Afoo + 172840, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_Afoo,172840 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_Afoo , 172840 + 32768);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 203608 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
