    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_Afoo , 0 , 65536 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				char *filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 65536, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				...
				while (moffset_Afoo < 32768){
					if (to_write_Afoo < 32){
						memcpy(filep_Afoo + 65536 + moffset_Afoo, mtext_Afoo, to_write_Afoo);
						moffset_Afoo += to_write_Afoo;
					}
					else {
						memcpy(filep_Afoo + 65536 + moffset_Afoo,mtext_Afoo, 32);
						moffset_Afoo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_Afoo + 65536, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_Afoo,65536 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_Afoo , 65536 + 32768);
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 98304) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);
				...
				if ( fallocate( fd_foo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 96304 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 163840) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);
				...
				if ( fallocate( fd_foo , 0 , 101304 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				 filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 101304, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				if (filep_foo == MAP_FAILED) {
					 return -1;
				}
				...
				while (moffset_foo < 32768){
					if (to_write_foo < 32){
						memcpy(filep_foo + 101304 + moffset_foo, mtext_foo, to_write_foo);
						moffset_foo += to_write_foo;
					}
					else {
						memcpy(filep_foo + 101304 + moffset_foo,mtext_foo, 32);
						moffset_foo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_foo + 101304, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_foo,101304 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_foo , 101304 + 32768);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 194608 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 199608) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 232376 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , 0 , 265144 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				 filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 265144, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				if (filep_Afoo == MAP_FAILED) {
					 return -1;
				}
				...
				while (moffset_Afoo < 32768){
					if (to_write_Afoo < 32){
						memcpy(filep_Afoo + 265144 + moffset_Afoo, mtext_Afoo, to_write_Afoo);
						moffset_Afoo += to_write_Afoo;
					}
					else {
						memcpy(filep_Afoo + 265144 + moffset_Afoo,mtext_Afoo, 32);
						moffset_Afoo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_Afoo + 265144, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_Afoo,265144 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_Afoo , 265144 + 32768);
