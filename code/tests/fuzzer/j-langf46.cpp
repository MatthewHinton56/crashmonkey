    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_KEEP_SIZE , 132072 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( WriteData ( fd_foo, 200608, 5000) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 202608 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
