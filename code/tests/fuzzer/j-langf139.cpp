    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 66536 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_KEEP_SIZE , 69536 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 140072, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				if ( fallocate( fd_foo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 172840 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
