    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 164840 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 233376 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				char *filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 163840, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE , 196608 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
