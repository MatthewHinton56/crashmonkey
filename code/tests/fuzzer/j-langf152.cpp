    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 63536 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , 0 , 68536 , 32768) < 0){ 
				 ...
				 filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 68536, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				if (filep_Afoo == MAP_FAILED) {
					 return -1;
				}
				...
				if ( WriteData ( fd_Afoo, 101304, 32768) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 132072 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE , 230376 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( WriteData ( fd_foo, 135072, 5000) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
