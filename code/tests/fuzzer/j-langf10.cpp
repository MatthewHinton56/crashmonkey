        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_Afoo , 0 , 32768 , 32768) < 0){ 
				 ...
				 filep_Afoo = (char *) cm_->CmMmap(NULL, 32768 + 32768, PROT_WRITE|PROT_READ, MAP_SHARED, fd_Afoo, 0);
				if (filep_Afoo == MAP_FAILED) {
					 return -1;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 65536 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 98304, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE , 131072 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 229376 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( WriteData ( fd_Afoo, 260144, 5000) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
