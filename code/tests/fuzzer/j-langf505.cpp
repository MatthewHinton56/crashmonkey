    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_KEEP_SIZE , 96304 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 96304 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 166840 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_KEEP_SIZE , 230376 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , 0 , 233376 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( WriteData ( fd_Afoo, 134072, 32768) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if ( WriteData ( fd_Afoo, 164840, 5000) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 166840 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
