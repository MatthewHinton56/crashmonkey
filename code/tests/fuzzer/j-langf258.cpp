    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( WriteData ( fd_foo, 68536, 32768) < 0){ 
					cm_->CmClose( fd_foo); 
					return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 129072 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , 0 , 137072 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				 filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 137072, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				if (filep_foo == MAP_FAILED) {
					 return -1;
				}
				...
				while (moffset_foo < 32768){
					if (to_write_foo < 32){
						memcpy(filep_foo + 137072 + moffset_foo, mtext_foo, to_write_foo);
						moffset_foo += to_write_foo;
					}
					else {
						memcpy(filep_foo + 137072 + moffset_foo,mtext_foo, 32);
						moffset_foo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_foo + 137072, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_foo,137072 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_foo , 137072 + 32768);
				...
				if ( fallocate( fd_foo , 0 , 169840 , 32768) < 0){ 
				 ...
				 filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 169840, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				if (filep_foo == MAP_FAILED) {
					 return -1;
				}
				...
				if ( WriteData ( fd_Afoo, 169840, 32768) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
