    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 33768 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 96304 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}


				if ( fallocate( fd_Afoo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 99304 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_ZERO_RANGE|FALLOC_FL_KEEP_SIZE , 36768 , 5000) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( pwrite ( fd_Afoo, data_Afoo, 32768, 104304) < 0){
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
				cm_->CmClose(fd_Afoo);
				...
				if ( fallocate( fd_foo , 0 , 41768 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				char *filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 41768, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				...
				while (moffset_foo < 32768){
					if (to_write_foo < 32){
						memcpy(filep_foo + 41768 + moffset_foo, mtext_foo, to_write_foo);
						moffset_foo += to_write_foo;
					}
					else {
						memcpy(filep_foo + 41768 + moffset_foo,mtext_foo, 32);
						moffset_foo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_foo + 41768, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_foo,41768 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_foo , 41768 + 32768);
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 135072 , 5000) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 140072 , 32768) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , FALLOC_FL_PUNCH_HOLE|FALLOC_FL_KEEP_SIZE , 74536 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				...
				if ( fallocate( fd_foo , 0 , 107304 , 32768) < 0){ 
					cm_->CmClose( fd_foo);
					 return errno;
				}
				 filep_foo = (char *) cm_->CmMmap(NULL, 32768 + 107304, PROT_WRITE|PROT_READ, MAP_SHARED, fd_foo, 0);
				if (filep_foo == MAP_FAILED) {
					 return -1;
				}
				...
				while (moffset_foo < 32768){
					if (to_write_foo < 32){
						memcpy(filep_foo + 107304 + moffset_foo, mtext_foo, to_write_foo);
						moffset_foo += to_write_foo;
					}
					else {
						memcpy(filep_foo + 107304 + moffset_foo,mtext_foo, 32);
						moffset_foo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_foo + 107304, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_foo,107304 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_foo , 107304 + 32768);
				...
				if ( WriteData ( fd_Afoo, 203608, 5000) < 0){ 
					cm_->CmClose( fd_Afoo); 
					return errno;
				}
