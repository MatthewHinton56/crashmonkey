...
[ $# -eq 0 ] && { echo "Usage: $0 filename" > "diff_temp.txt"; exit 1; }
[ ! -f "$_file" ] && { echo "Error: $0 file not found." > "diff_temp.txt"; exit 2; }
 
if [ -s "$_file" ] 
then
	echo -e "${red}${bold} : Failed test${reset}"
	echo -e "Failed test" > "diff_temp.txt"
	...
	then
		...
		echo -e "Passed test" > "diff_temp.txt"
	else	
		echo -e "${yellow}${bold} : Could not run test${reset}"
		echo -e "Could not run test" > "diff_temp.txt"
	fi
fi
