...
def build_parser():
    ...
    # global args
    parser.add_argument('--file_path', '-p', default='', help='Path to workload')
    parser.add_argument('--snapshot', '-s', default='-1', help="The snapshot it crashed on")
...
CheckPointOptions = ['fdatasync', 'sync', 'fsync', 'mmapwrite']


def findAndRemovePostCheckpoint(seq, checkpoint):
    local_checkpoint = 0
    for index in range(0, len(seq)):
        op = seq[index][0]
        if op in CheckPointOptions:
            local_checkpoint += 1
        if checkpoint == local_checkpoint:
            newList = list()
            for i in range(0, len(seq[index]) - 1):
                newList.append(seq[index][i])
            newList.append('1')
            seq[index] = tuple(newList)
            return seq[0:(index+1)]

def create_trim_workloads(seq, jlang):
    global FileOptions
    global SecondFileOptions
    global SecondDirOptions
    global OperationSet
...
def generateJLang(modified_sequence, num):
    j_lang_file = 'j-langt' + str(num)
    ...
    f.close()
    exec_command = 'python ./ace/cmAdapter.py -b ./code/tests/' + dest_dir + '/base.cpp -t ' + j_lang_file + ' -p ./code/tests/' + dest_dir + '/ -o ' + str(num)
