    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				int moffset_foo = 0;
				int mto_write_foo = 32768 ;
				const char *mtext_foo  = "mmmmmmmmmmklmnopqrstuvwxyz123456";

				while (moffset_foo < 32768){
					if (mto_write_foo < 32){
						memcpy(filep_foo + 32768 + moffset_foo, mtext_foo, mto_write_foo);
						moffset_foo += mto_write_foo;
					}
					else {
				...
				if ( WriteData ( fd_ACfoo, 0, 32768) < 0){ 
					cm_->CmClose( fd_ACfoo); 
					return errno;
				}


				if ( fallocate( fd_ACfoo , 0 , 32768 , 32768) < 0){ 
					cm_->CmClose( fd_ACfoo);
					 return errno;
				}
				char *filep_ACfoo = (char *) cm_->CmMmap(NULL, 32768 + 32768, PROT_WRITE|PROT_READ, MAP_SHARED, fd_ACfoo, 0);
				if (filep_ACfoo == MAP_FAILED) {
					 return -1;
				}

				int moffset_ACfoo = 0;
				int mto_write_ACfoo = 32768 ;
				const char *mtext_ACfoo  = "mmmmmmmmmmklmnopqrstuvwxyz123456";

				while (moffset_ACfoo < 32768){
					if (mto_write_ACfoo < 32){
						memcpy(filep_ACfoo + 32768 + moffset_ACfoo, mtext_ACfoo, mto_write_ACfoo);
						moffset_ACfoo += mto_write_ACfoo;
					}
					else {
						memcpy(filep_ACfoo + 32768 + moffset_ACfoo,mtext_ACfoo, 32);
						moffset_ACfoo += 32; 
					} 
				}

				if ( cm_->CmMsync ( filep_ACfoo + 32768, 8192 , MS_SYNC) < 0){
					cm_->CmMunmap( filep_ACfoo,32768 + 32768); 
					return -1;
				}
				cm_->CmMunmap( filep_ACfoo , 32768 + 32768);
